...
function App() {
  ...
  let handleDelete=(e)=>{
    ...
    var delete1 = parseInt(e.target.closest('tr').querysSelector('td:first-child').innerHTML);
    console.log(delete1);
    swal({
      title: "Are you sure?",
    ...
    .then(async (willDelete) => {
      if (willDelete) {
       try {
         //api call
         let po = await  axios.delete(`${URL}/api/friends/${delete1}`);
         tr.remove();
                   ...
                   return (
                       <tr key={index}>
                         ...
                         <td>
                           ...
                           <Button variant="danger" size="sm" onClick={(e)=>{handleDelete(e)}}>Delete</Button>
                         </td>
                       </tr>
                   )
                 })
